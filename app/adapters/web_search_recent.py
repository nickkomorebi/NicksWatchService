...
class WebSearchRecentAdapter(BaseAdapter):
            ...
            for query in build_queries(watch):
                ...
                try:
                    resp = await client.post(
                        SERPER_URL, json={"q": q, "num": 20, "tbs": "qdr:w"}, headers=headers
                    )
                    resp.raise_for_status()
                except httpx.HTTPError as exc:
                    raise AdapterError(f"Serper (recent) request failed: {exc}") from exc
